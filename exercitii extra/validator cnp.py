...
an='1234567890'
cnp=input('enterCNP:')
cnpc= int(2 * cnp[0] + 7 * cnp[1] + 2 * cnp[2] + 2 * cnp[3] + 2 * cnp[4] + 2 * cnp[5] +
          2 * cnp[6] + 2 * cnp[7] + 2 * cnp[8] + 2 * cnp[9] + 2 * cnp[10] + 2 * cnp[11]+ 2 * cnp[12]) % 11
date= cnp[1:7]
...
def cnpval():
    if datetime.datetime.strptime(date,"%y%m%d"):
        if len(cnp) == 13:
            if cnp[0] in an:
                if re.match(valid,cnp[7:]):
                    if cnp[12]==cnpc:     #aici crapa, trebuie testat
                        return True
    else:
         return False
if cnpval()==True:
